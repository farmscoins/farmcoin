This directory contains the FarmcoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Farmcoin UI.
...
ClientModel is used by the main application `farmcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/farmcoin/farmcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a farmcoin:... link from a browser).
...
* UI elements like FarmcoinAmountField, which inherit from QWidget.
* `farmcoinstrings.cpp`: automatically generated
* `farmcoinunits.(h/cpp)`: FMC / mFMC / etc handling
* `callback.h`
...
* don't change `local/farmcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "farmcoin-qt" under "Run", which is where you can also add command line arguments)
