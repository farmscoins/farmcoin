...
const char * const FARMCOIN_CONF_FILENAME = "farmcoin.conf";
const char * const FARMCOIN_PID_FILENAME = "farmcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "farmcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Farmcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Farmcoin
    // Mac: ~/Library/Application Support/Farmcoin
    // Unix: ~/.farmcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Farmcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Farmcoin";
#else
    // Unix
    return pathRet / ".farmcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", FARMCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", FARMCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Farmcoin Core") == std::string::npos) {
