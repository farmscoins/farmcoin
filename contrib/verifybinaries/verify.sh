...
###   farmcoincore.org and farmcoin.org and compares them.
...
WORKINGDIR="/tmp/farmcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://farmcoincore.org"
HOST2="https://farmcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="farmcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'farmcoin-',
...
if [ $? -ne 0 ]; then
   echo "farmcoin.org failed to provide signature file, but farmcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "farmcoin.org and farmcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Farmcoin Core binary release signing key installed?"
   fi
