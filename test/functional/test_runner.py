...
`test/functional/test_framework/test_framework.py:FarmcoinTestFramework.main`.
...
def main():
    ...
    enable_farmcoind = config["components"].getboolean("ENABLE_FARMCOIND")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/farmcoin/farmcoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/farmcoin/farmcoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not enable_farmcoind:
...
def run_tests(test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False):
    ...
    # Warn if farmcoind is already running (unix only)
    try:
...
class RPCCoverage():
    ...
    commands per `farmcoin-cli help` (`rpc_interface.txt`).
